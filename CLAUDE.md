...
## The agent list on platform-agents.html is generated

The table of agents between `<!-- BEGIN generated by
tools/build-platform-agents.mjs -->` and `<!-- END generated -->` is written
from the platform-agent registry in the AgentMesh repo
(`services/src/api/platform-agent-registry.ts`), which is where a platform
agent is declared in the first place. Do not hand-edit the block, and do not
add an agent here: add it to the registry with its `group`, `blurb` and
`reach`, then regenerate.

```bash
node tools/build-platform-agents.mjs   # from the repo root
```

Everything else on the page is hand-written and the script leaves it alone.
Commit the regenerated page, because deploy.sh ships committed files only.
